...
class EdgeConv(MessagePassing):
    def __init__(self, in_channels, edge_channels, out_channels):
        ...
        self.lin_1 = Linear(edge_channels, edge_channels, bias=True)
    ...
    def forward(self, x, edge_index, edges):
        ...
        lisa_print(edges)
        # eout = self.lin_e(edges)
        out = self.lin_1(edges )
        out = self.lin_f(out )
        # lisa_print(out)
        return out
    def aggregate(self, inputs, x,edge_index,  x_i: Tensor, x_j: Tensor) -> Tensor:

        return  self.lin_1(self.lin_p(x_i) + self.lin_c(x_j))
    



class Net(torch.nn.Module):
    def __init__(self, in_channels, edge_channels, hidden_channels, num_layers):
...
def validation(model, val_dataset, device):  # For validation, the input data is WHOLE VALIDATION DATASET.
    ...
    for data in val_dataset:
        ...
        correct += int(pred.eq(y+2).sum().item())
        correct += int(pred.eq(y-2).sum().item())
        correct += int(pred.eq(y-1).sum().item())
...
def label4_inference(data: Data, infer_model_name = "final_model"):
    device = torch.device('cpu')
    final_model_path  = pathlib.Path().absolute()
    final_model_path = os.path.join(final_model_path, "Label4/checkpoint/"+infer_model_name+".pt")
    m_model = Net(2, 5, 1, 1).to(device)
